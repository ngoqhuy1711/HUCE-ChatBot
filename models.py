...
class AdvancedChatRequest(BaseModel):
    ...
    message: str = Field(..., min_length=1, description="Câu hỏi từ người dùng")
    session_id: Optional[str] = Field(default="default", description="ID phiên hội thoại")
    use_context: Optional[bool] = Field(default=True, description="Có sử dụng context không")
...
class ContextRequest(BaseModel):
    ...
    context: Optional[Dict[str, Any]] = Field(default=None, description="Context mới (chỉ dùng khi action=set)")
...
class SuggestMajorsRequest(BaseModel):
    ...
    score_type: Optional[str] = Field(default="chuan", description="Loại điểm: 'chuan' hoặc 'san'")
...
class BaseResponse(BaseModel):
    ...
    message: Optional[str] = Field(default=None, description="Thông điệp cho người dùng")
...
class DataResponse(BaseResponse):
    ...
    data: List[Dict[str, Any]] = Field(default_factory=list, description="Dữ liệu trả về")
...
class NLPAnalysisResponse(BaseResponse):
    ...
    entities: List[Dict[str, Any]] = Field(default_factory=list, description="Entities trích xuất")
...
class ChatResponse(BaseResponse):
    ...
    context: Optional[Dict[str, Any]] = Field(default=None, description="Context sau khi xử lý")


class ContextResponse(BaseResponse):
    """Response cho context management"""
    context: Dict[str, Any] = Field(default_factory=dict, description="Context hiện tại")
