...
def create_success_response(data: Optional[List[Dict[str, Any]]] = None, message: Optional[str] = None) -> Dict[
    str, Any]:
    if data is not None:
        return DataResponse(success=True, message=message, data=data, count=len(data)).model_dump()
...
def create_error_response(message: str, error_code: Optional[str] = None, error_detail: Optional[str] = None) -> Dict[
    str, Any]:
    return ErrorResponse(success=False, message=message, error_code=error_code, error_detail=error_detail).model_dump()
