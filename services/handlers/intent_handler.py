...
from services.processors import (
    infer_major_from_message, find_standard_score, list_majors, list_tuition,
    list_scholarships, list_admission_conditions, list_admission_quota,
    list_admission_methods_general, list_admission_methods, list_admissions_schedule,
    get_admission_targets, get_combination_codes, get_combination_by_code,
    format_data_to_text, add_contact_suggestion, clean_program_name,
)
...
def _compose_message(intro: str = "", formatted_text: str = "", outro: str = "", include_contact: bool = False) -> str:
    """Ghép các phần của phản hồi thành một đoạn hội thoại."""
...
def _build_data_response(response_type: str, results: list, intro: str, formatted_text: str, empty_hint: str,
                         outro: str = DEFAULT_OUTRO) -> Dict[str, Any]:
    """Tạo response cho intent có dữ liệu."""
    if results:
...
def handle_intent_query(analysis: Dict[str, Any], context: Dict[str, Any], original_message: str = "") -> Dict[
    ...
    # Context inference
    is_general_query = any(kw in original_message.lower() for kw in
                           ["tất cả", "tat ca", "các ngành", "cac nganh", "chung", "toàn bộ",
                            "toan bo"]) if original_message else False
    is_followup = any(kw in original_message.lower() for kw in
                      ["còn", "con", "thêm", "them", "nữa", "nua", "khác", "khac"]) if original_message else False
    ...
    if intent.startswith("hoi_diem_chuan"):
        ...
        return {"type": "fallback", "message": _compose_message(
...
def _handle_diem_chuan(major_info, year_info):
    if major_info:
        ...
        intro = f"Mình tìm được {len(results)} kết quả điểm chuẩn của ngành {major_info} năm {year_label}." if results else ""
        empty_hint = f"Mình chưa thấy dữ liệu điểm chuẩn cho ngành {major_info}. Bạn thử kiểm tra lại tên ngành hoặc hỏi mình về năm khác."
        return _build_data_response("standard_score", results, intro, format_data_to_text(results, "standard_score"),
                                    empty_hint)
    return {"type": "clarification",
            "message": _compose_message("Bạn cho mình xin tên ngành bạn quan tâm để mình tra điểm chuẩn giúp nhé?",
                                        include_contact=True)}
...
def _handle_nganh_hoc(major_info):
    if major_info:
        ...
        return _build_data_response("major_info", results, intro, format_data_to_text(results, "major_info"),
                                    empty_hint)
    return {"type": "clarification", "message": _compose_message(
...
def _handle_hoc_phi(major_info, year_info):
    ...
    intro = " ".join(intro_parts) + " mà mình tìm được." if results else "Đây là thông tin học phí mới nhất mà mình có."
    empty_hint = "Mình chưa có dữ liệu học phí để chia sẻ ngay lúc này."
    return _build_data_response("tuition", results, intro, format_data_to_text(results, "tuition"), empty_hint)


def _handle_hoc_bong():
    results = list_scholarships()
    international_kw = ["Anh", "Bỉ", "Ý", "Pháp", "Đức", "Slovakia", "Hoa Kỳ", "Mexico", "Canada", "Australia",
    ...
    domestic = [s for s in results if not any(kw in s.get('scholarship_name', '') for kw in international_kw)]
    international = [s for s in results if any(kw in s.get('scholarship_name', '') for kw in international_kw)]
    ...
    return _build_data_response("scholarships", results, intro, "\n".join(lines),
...
def _handle_dieu_kien(year_info):
    ...
    return _build_data_response("admission_conditions", results, intro,
...
def _handle_chi_tieu(major_info, year_info):
    ...
    return _build_data_response("admission_quota", results, intro, format_data_to_text(results, "admission_quota"),
                                empty_hint)


def _handle_phuong_thuc(major_info, original_message):
    search_major = major_info or (infer_major_from_message(original_message) if original_message else None)
    if not search_major:
        ...
        return _build_data_response("admission_methods_general", results, intro,
    ...
    return _build_data_response("admission_methods", results, intro, format_data_to_text(results, "admission_methods"),
...
def _handle_thoi_gian_dk(entities):
    phuong_thuc = None
    for e in entities:
        if e.get("label") in ["PHUONG_THUC", "PHUONG_THUC_XET_TUYEN", "PHUONG_THUC_TUYEN_SINH"]:
    ...
    return _build_data_response("admissions_schedule", results, intro,
                                format_data_to_text(results, "admissions_schedule"), empty_hint)


def _handle_to_hop_mon(major_info, original_message=""):
    import re
    combo_pattern = r'\b([A-Z]\d{2}|[A-Z]{2}\d|SP\d|VS\d|TT)\b'
    combo_matches = re.findall(combo_pattern, original_message.upper()) if original_message else []
    ...
    if combo_matches:
        ...
        if results:
            ...
            return _build_data_response("combination_details", results, intro,
                                        format_data_to_text(results, "combination_details"), "")

    # General list query
    if not major_info:
        if original_message and any(kw in original_message.lower() for kw in
    ...
    if len(major_info) != 7:
        ...
        targets = [t for t in targets if
                   mq in t.get("major_name", "").lower() or mq in t.get("program_name", "").lower()]

    if not targets:
        return {"type": "major_combo", "data": [],
                "message": _compose_message(f"Mình chưa tìm thấy tổ hợp môn cho ngành {major_info}.",
                                            include_contact=True)}

    combo_details = {r.get("combination_code"): {"subjects": r.get("subject_names", ""), "note": r.get("note", "")} for
                     r in get_combination_codes()}
    ...
    for t in targets:
        ...
        if prog_name not in programs:
            programs[prog_name] = {"program_name": prog_name, "major_code": t.get("major_code", "N/A"), "methods": {}}
        ...
        if data["methods"]:
            for method_code, combos in sorted(data["methods"].items()):
                ...
                if len(ml) == 1:
                    ...
                    method_display = f"{abbr} - {name}" if abbr and name else abbr or name or f"Phương thức {method_code}"
                elif len(ml) > 1:
    ...
    message = _compose_message(f"Các tổ hợp môn áp dụng cho ngành {major_info}.", "\n".join(lines), DEFAULT_OUTRO)
