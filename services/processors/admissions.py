...
def list_admission_quota(major: Optional[str] = None, year: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    for t in targets:
        ...
        if key not in major_quotas:
            major_quotas[key] = {"major_code": key[0], "major_name": key[1], "nam": "2025", "chi_tieu": 0,
                                 "chi_tiet": []}
        try:
...
def list_admission_methods_general() -> List[Dict[str, Any]]:
    ...
    return [{"method_code": r.get("method_code"), "abbreviation": r.get("abbreviation"),
             "method_name": r.get("method_name"), "description": r.get("description"),
             "requirements": r.get("requirements")} for r in rows]


def list_admission_methods(major: Optional[str] = None) -> List[Dict[str, Any]]:
    """Tìm kiếm phương thức xét tuyển theo ngành."""
    targets = get_admission_targets(ma_nganh=major if major and len(major) == 7 else None)
    ...
    for m in list_admission_methods_general():
        ...
        if code:
            method_mapping.setdefault(code, []).append({
                "method_name": m.get("method_name", ""), "abbreviation": m.get("abbreviation", ""),
    ...
    for t in targets:
        ...
        if key not in methods_map:
            ...
            if len(ml) > 1:
                method_name = " / ".join(
                    [f"{m['abbreviation']} - {m['method_name']}" for m in ml if m['abbreviation'] and m['method_name']])
            ...
            methods_map[key] = {
                "major_code": key[0], "major_name": t.get("major_name", ""), "admission_method": key[1],
                "method_code": key[1], "method_name": method_name, "abbreviation": abbreviation,
                "description": method_desc, "subject_combination": []}
    ...
    return [dict(d, subject_combination=", ".join(d["subject_combination"])) for d in methods_map.values()]


def list_admissions_schedule(phuong_thuc: Optional[str] = None) -> List[Dict[str, Any]]:
    """Tìm kiếm lịch trình xét tuyển."""
        ...
        if phuong_thuc:
            if pt_raw.lower() in ["tất cả", "tat ca"]:
                ...
                if phuong_thuc.lower() not in pt_raw.lower():
                    continue
        results.append({"event_name": event_name, "timeline": r.get("timeline") or "",
                        "admission_method": pt_raw, "note": r.get("note") or ""})
    return results
...
def get_admission_targets(ma_nganh: Optional[str] = None, phuong_thuc: Optional[str] = None,
    ...
    for row in rows:
        ...
        if phuong_thuc and row.get("admission_method", "").strip() != phuong_thuc.strip():
            continue
        if to_hop:
            to_hop_list = [x.strip() for x in row.get("subject_combination", "").split(",")]
...
def get_combination_codes(ky_thi: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    if ky_thi:
        return [r for r in rows if ky_thi.strip() in [x.strip() for x in r.get("exam_type", "").split(",")]]
    return rows


def get_combination_by_code(combo_code: str) -> List[Dict[str, Any]]:
    """Tìm thông tin chi tiết tổ hợp môn theo mã."""
    rows = read_csv(os.path.join(DATA_DIR, "subject_combinations.csv"))
    code_upper = combo_code.strip().upper()
    return [r for r in rows if (r.get("combination_code") or "").strip().upper() == code_upper]


def search_combinations(query: str) -> List[Dict[str, Any]]:
    """Tìm kiếm tổ hợp môn theo mã hoặc tên môn."""
    rows = read_csv(os.path.join(DATA_DIR, "subject_combinations.csv"))
