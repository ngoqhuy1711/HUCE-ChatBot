...
def canonicalize_vi_ascii(text: str) -> str:
    """Chuẩn hóa biến thể thường gặp sau khi bỏ dấu."""
    if not isinstance(text, str):
        return ""
...
def clean_program_name(name: str) -> str:
    """Nếu tên ở dạng 'Ngành/Chuyên ngành' → chỉ giữ 'Chuyên ngành'."""
...
def infer_major_from_message(message: str) -> Optional[str]:
    """Suy luận tên ngành từ message khi entity extractor không bắt được."""
...
def _get_method_name_mapping() -> Dict[str, str]:
    """Tạo mapping từ method_code sang tên phương thức đầy đủ."""
...
def format_data_to_text(data: List[Dict[str, Any]], data_type: str) -> str:
    """Format data thành text để hiển thị."""
    if not data:
    ...
    if data_type == "standard_score":
        ...
        for item in data:
            ...
            if program not in grouped:
                grouped[program] = {'combination': item.get('subject_combination', 'N/A'), 'scores': []}
            grouped[program]['scores'].append({'year': item.get('nam', 'N/A'), 'score': item.get('diem_chuan', 'N/A')})
